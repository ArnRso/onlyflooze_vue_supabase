          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="tx in filteredTransactions" :key="tx.id">
              <td class="px-4 py-2 text-center">
                <input v-model="selectedIds" type="checkbox" :value="tx.id" />
              </td>
...
<script lang="ts" setup>
  ...
  import { useAddCategoryMutation, useCategoriesQuery } from '@/queries/useCategories'
  ...
  const canAssign = computed(
    () => selectedIds.value.length > 0 && selectedCategory.value && selectedCategory.value.id
  )
  ...
  async function assignCategory() {
    if (!selectedCategory.value || !selectedCategory.value.id || selectedIds.value.length === 0)
      return
    try {
