...
<template>
  ...
  >
    <div class="max-w-7xl w-full mx-auto bg-white rounded-xl shadow-md overflow-hidden p-8">
      <h1 class="text-3xl font-extrabold text-gray-900 mb-6 text-center">Tags</h1>
      ...
      <ul>
        ...
        >
          ...
          <div v-else class="flex gap-2 flex-1">
            ...
            <button class="bg-green-600 text-white px-2 py-1 rounded shadow" @click="handleEdit">
              Valider
            </button>
            <button
          ...
          <div v-if="editId !== tag.id" class="flex gap-2">
            <button class="text-blue-600 hover:underline" @click="() => startEdit(tag)">
              Modifier
            </button>
            <button class="text-red-600 hover:underline" @click="() => handleDelete(tag.id)">
              Supprimer
            </button>
          </div>
        </li>
