  ...
  >
    <div class="max-w-md w-full mx-auto bg-white rounded-xl shadow-md overflow-hidden p-8">
      <div class="text-center mb-8">
        <h2 class="text-3xl font-extrabold text-gray-900 mb-2">Profil utilisateur</h2>
      ...
      <div v-if="user">
        <ul class="space-y-4">
          ...
          <li v-if="profile?.username">
            ...
            <span class="block text-lg text-gray-900">{{ profile.username }}</span>
          </li>
          <li v-if="profile?.website">
            <span class="block text-sm font-medium text-gray-700">Site web</span>
            <span class="block text-lg text-indigo-600 underline">{{ profile.website }}</span>
          </li>
        </ul>
      </div>
