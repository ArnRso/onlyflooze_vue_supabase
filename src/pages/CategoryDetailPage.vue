...
const {
  ...
  isLoading,
  error,
} = useCategoryTransactionsQuery(categoryId.value)
...
function getAllMonthsLabels() {
  ...
  while (current <= end) {
    labels.push(
      current.toLocaleString('fr-FR', { month: 'short', year: '2-digit' })
    )
    ...
    >
      ...
      <div v-else>
        <div class="flex items-center justify-between mb-6">
          ...
          <h1
            class="text-3xl font-extrabold text-gray-900 text-center flex-1 m-0"
          >
            {{ category.label }}
          </h1>
          ...
          <div class="mb-4 p-4 bg-indigo-50 rounded">
            ...
            <div>
              ...
              <span class="font-mono">{{
                recurringEstimate.estimatedDay
              }}</span>
            </div>
            <div>
              Montant estimé :
              <span class="font-mono"
                >{{ recurringEstimate.nextAmount }} €</span
              >
            </div>
          </div>
        </div>
        <div class="mb-6">
          <table
            class="min-w-full divide-y divide-gray-200 bg-gray-50 rounded text-sm"
          >
            <thead>
              <tr>
            ...
            <tbody>
              <tr>
                <td class="px-4 py-2 font-mono">
                  {{ formatAmount(totalMoisEnCours) }}
                </td>
                <td class="px-4 py-2 font-mono">
                  {{ formatAmount(totalMoisPrecedent) }}
                </td>
                <td class="px-4 py-2 font-mono">
                  {{ formatAmount(totalAnneeEnCours) }}
                </td>
                <td class="px-4 py-2 font-mono">
                  {{ formatAmount(totalAnneePrecedente) }}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <ul v-else class="divide-y divide-gray-200">
          ...
          >
            ...
            <span class="flex-1 text-gray-700 mx-4 truncate">{{
              tx.label
            }}</span>
